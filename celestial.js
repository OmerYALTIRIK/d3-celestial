      ...
      if (cfg.stars.show) {
        ...
        container.selectAll(parentElement + " .star").each(function (d) {
          if (clip(d.geometry.coordinates) && d.properties.mag <= cfg.stars.limit) {
            ...
            // Add glow effect
            var starGlowColor = starColor(d); // get the color of the star
            const steps = Math.round(5 * adapt);
            for (var i = 1; i <= steps; i++) {
              context.beginPath();
              context.arc(pt[0], pt[1], r + (i * (7.5 / steps) * adapt), 0, 2 * Math.PI);
              context.globalAlpha = (0.005 * steps) / i; // decrease opacity with each layer
              context.shadowBlur = i * (7, 5 / steps) * adapt; // introduce blur to the glow effect
              context.shadowColor = starGlowColor; // use the star's color for the glow effect
              context.fillStyle = starGlowColor; // set the fill color to the star's color
              context.fill();
            }
            // Reset global alpha
            context.globalAlpha = 1;
            ...
            context.beginPath();
            context.arc(pt[0], pt[1], r * 1.2, 0, 2 * Math.PI); // slightly larger radius for glow
            context.closePath();
            context.fill();
